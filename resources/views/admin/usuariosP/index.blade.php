							...
							@foreach($user as $users)    
								@if($users->status == 0)
									@if($users->id != Auth::user()->id)
										<?php $i++; ?>	
							<tr>
								<td><img src="{{asset('uploads/usuarios/'.$users->imagen)}}" alt=""> </td>
								<td>{{$users->name}}</td>
								<td>{{$users->email}}</td>
								<td>{{$users->rol}}</td>
								<td>@if($users->status==1) <i class="badge">Pausado</i>@endif</td>
								...
								<td>
									<div class="row">
										...
										<div class="col-md-6 col-xs-6">
											<a href="#" class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="left" title="Activar"><i class="fa fa-play"></i></a>
